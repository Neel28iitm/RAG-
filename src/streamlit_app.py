...
def get_services():
    ...
    return config, retrieval_service, generation_service, history_manager, query_classifier
...
def main():
    ...
    # Load Services
    try:
        config, retrieval_service, generation_service, history_manager, query_classifier = get_services()
    except Exception as e:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                try:
                    ...
                    # RAG Pipeline with Query Classification
                    
                    # Classify query type
                    query_type = query_classifier.classify(prompt)
                    
                    if query_type == 'generic':
                        # FAST PATH: Generic queries with streaming
                        import time
                        ...
                        # Generic responses are short, simulate streaming for consistency
                        complete_response = generation_service.generate_generic_response(
                            prompt,
                            chat_history=lc_history[-6:] if lc_history else []
                        )
                        ...
                        docs = []  # No docs for generic
                        
                    else:
                        # FULL RAG: Knowledge queries
                        ...
                        docs, metrics = retrieval_service.get_relevant_docs(
                            prompt, 
                            top_k=min(15, config['retrieval']['top_k']),
